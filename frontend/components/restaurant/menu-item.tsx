...
export function MenuItem({ menuItem, onAddToCart, disabled = false }: MenuItemProps) {
  const quantity = useCartStore((state) =>
    state.items.find((item) => item.menuItem.id === menuItem.id)?.quantity || 0
  )
    ...
    >
      ...
      <div className="relative w-24 h-24 shrink-0">
        ...
        {quantity > 0 && (
          <Badge
            variant="default"
            className="absolute -top-2 -right-2 h-6 w-6 rounded-full p-0 flex items-center justify-center text-xs font-semibold shadow-lg"
